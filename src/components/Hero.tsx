...
export const Hero: React.FC<HeroProps> = ({
  ...
  // Dynamic countdown logic based on the user's HTML implementation
    ...
    const updateHeroCountdown = () => {
      ...
      if (diffToLaunch > 0) {
        ...
        if (isSaturdayNight || isSunday) {
          ...
          const diffWeekly = targetSaturday.getTime() - now.getTime();
  ...
  const availableCount = spotsList.filter((s) => !s.sponsor).length;
  const fundingPct = Math.min(100, Math.round((totalRaised / 1000) * 100));
  ...
  return (
    <section className="max-w-[1160px] mx-auto px-4 sm:px-6 pt-5 sm:pt-6 pb-6 relative" id="hero">
      {/* Top Row: Dynamic Countdown Widget */}
        ...
        >
          ...
          <span
            className={`w-1.5 h-1.5 rounded-full ${
              isLivePulse ? 'bg-[#10b981] animate-pulse' : 'bg-neutral-400'
            }`}
      ...
      {/* Side-by-Side Front & Back Standing Figures */}
      <div className="relative w-full max-w-4xl mx-auto">
          ...
          <div className="relative flex flex-col items-center">
            ...
            >
              ...
              {frontSpotKeys.map((key) => {
                ...
                if (spot.sponsorUrl) {
                  try {
                    domain = new URL(spot.sponsorUrl).hostname.replace(/^www\./, '');
                  } catch {
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer absolute rounded transition-all duration-200 border group z-10 flex items-center justify-center p-0.5 ${
                      isSponsored
                  ...
                  >
                    ...
                    >
                      {spot.logoUrl && (
                        <img
                          ...
                          className="w-full h-full object-contain pointer-events-none drop-shadow-sm transition-transform duration-200 group-hover:scale-105"
                        />
                      )}
                    </div>
                    <span
                    ...
                    >
                      {isSponsored ? (
                        <span>
                          Sponsored by <strong className="text-white">{spot.sponsor}</strong> • Click to visit{' '}
                          <span className="text-emerald-300 underline font-mono">{domain} ↗</span>
                        </span>
                      ) : (
                        <span>{spot.name} · Available · Bid now</span>
                      )}
                    </span>
                  </button>
                );
        ...
        <div className="mt-3 flex justify-between items-center px-2 text-[10px] uppercase font-semibold tracking-wider text-[#999999]">
          ...
          <span className="flex items-center gap-1.5 text-[#0f0f0f]">
            ...
            <span id="spots-count-label">{availableCount} OF 15 SPOTS AVAILABLE</span>
          </span>
        </div>
      </div>

      {/* Raised Amount / Goal Card */}
      <div className="max-w-xl mx-auto mt-6 p-4 rounded-xl border border-[#e5e5e0] bg-white shadow-xs">
        <div className="flex items-baseline justify-between mb-2">
          <div className="flex items-baseline gap-2">
            <span className="text-xl font-bold text-[#0f0f0f]" id="raised-amount">
              ${totalRaised}
            </span>
            <span className="text-xs text-[#666666] font-medium">raised of $1,000 goal</span>
          </div>
          <span
