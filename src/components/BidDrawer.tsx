...
import { createFallbackTextLogo } from '../data/spotsData';
...
interface BidDrawerProps {
  ...
  spot: Spot | null;
  onClose: () => void;
  onSubmitBid: (values: BidFormValues) => void;
}
...
export const BidDrawer: React.FC<BidDrawerProps> = ({
  ...
  const currentMinBid = currentBid > 0 ? currentBid * 2 : 10;
  ...
  // Before first bid: Current = $0, Minimum = $10, Next Minimum = $20.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Please enter a valid website URL (e.g. abc.com).');
      return;
    }
    ...
    const finalLogo = logoUrl || createFallbackTextLogo(brandName);
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded ${
                  isSponsored ? 'bg-emerald-100 text-emerald-700' : 'bg-neutral-100 text-neutral-600'
                }`}
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-4" id="bid-drawer-form">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0"
                  className={`w-full pl-9 pr-4 py-3 rounded-xl border-2 font-mono text-2xl font-bold text-neutral-900 placeholder:text-neutral-300 focus:outline-none transition-colors ${
                    isBelowMin
            ...
            <div className="pt-1">
              <button
                type="submit"
                disabled={!isValidBid}
                className={`w-full py-3 px-4 rounded-xl text-sm font-bold transition-all shadow-md active:scale-98 cursor-pointer flex items-center justify-center gap-2 ${
                  isValidBid
